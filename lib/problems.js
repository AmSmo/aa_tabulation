...
function stepper(nums) {
    ...
    for (let i = 0; i < nums.length; i++) {
        ...
        let maxSteps = nums[i]
        for (let num = 0; num <= maxSteps; num++) {
            table[i + num] = true
        }
    }
...
//     let table = new Array(nums).fill(0)
//     table[0] = nums[0]
//     for (let i = 1; i < nums.length; i++) {
//         let currentNum = nums[i]
//         let skipLeft = table[i - 2] === undefined ? 0 : table[i - 2]
//         let includeThis = skipLeft + currentNum
//         let includeLeft = table[i - 1]
//         let most = Math.max(includeThis, includeLeft)
//         table[i] = most
//     }
//     return table[table.length - 1]
// }
...
function maxNonAdjacentSum(nums, memo = {}) {
    ...
    memo[nums.length] = Math.max(nums[0] + maxNonAdjacentSum(nums.slice(2), memo), maxNonAdjacentSum(nums.slice(1), memo))
...
//
// You've seen this problem before with memoization, but now solve it using the Tabulation Stringategy!
//
// Examples:
//
